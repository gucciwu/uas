...
import com.mszq.platform.app.uas.service.IUasRoleTypeService;
...
@Controller
@RequestMapping("/uasRoleType")
public class UasRoleTypeController extends BaseController {
	private Logger logger = LoggerFactory.getLogger(this.getClass());

	@Autowired(required = true)
	private IUasRoleTypeService uasRoleTypeService;
	...
	private EUDataGridResult list(@RequestParam(value = "page") Integer page,
		...
		try {
			res = uasRoleTypeService.queryAll(page, rows);
		} catch(Exception e) {
			logger.error("查询出错", e);
		}
		return res;
	...
	@ResponseBody
	private List<UasRoleType> comList() throws Exception {
		List<UasRoleType> res = null;
		try {
			res = uasRoleTypeService.query();
		} catch(Exception e) {
			logger.error("查询出错", e);
		}
		return res;
	...
	@ResponseBody
	public CustomResult updateRoleType(UasRoleType record) {
		int ret = -1;
		try {
			ret = uasRoleTypeService.update(record);
		} catch(Exception e) {
			logger.error("更新出错", e);
		}
	...
	public CustomResult deleteRoles(String[] ids) {
		int ret =  -1;
		try {
			ret = uasRoleTypeService.deleteBatch(ids);
		} catch(Exception e) {
			logger.error("删除出错", e);
		}
	...
	@ResponseBody
	public CustomResult insert(UasRoleType record) {
		int ret =  -1;
		try {
		    ret =  uasRoleTypeService.insert(record);
		} catch(Exception e) {
			logger.error("保存出错", e);
		}
