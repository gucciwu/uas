...
import com.mszq.platform.app.uas.service.IUasSessionService;
...
@Controller
@RequestMapping("/uasSession")
public class UasSessionController {
	private Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired(required = true)
	private IUasSessionService uasSessionService;
	...
	private EUDataGridResult list(@RequestParam Map<String, Object> condition, @RequestParam(value = "page") Integer page,
		...
		try {
			res = uasSessionService.selectList(condition, page, rows);
		} catch(Exception e) {
			logger.error("查询出错", e);
		}
		return res;
	...
	@ResponseBody
	public CustomResult update(UasSession record) {
		int ret = -1;
		try {
			ret = uasSessionService.update(record);
		} catch(Exception e) {
			logger.error("更新出错", e);
		}
	...
	public CustomResult deleteRoles(String[] ids) {
		int ret =  -1;
		try {
			ret = uasSessionService.deleteBatch(ids);
		} catch(Exception e) {
			logger.error("删除出错", e);
		}
	...
	@ResponseBody
	public CustomResult insert(UasSession record) {
		int ret =  -1;
		try {
		    ret =  uasSessionService.insert(record);
		} catch (Exception e) {
			logger.error("保存出错", e);
		}
