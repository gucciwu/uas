...
import com.mszq.platform.app.uas.service.IUasAppService;
...
@Controller
@RequestMapping("/uasApp")
public class UasAppController extends BaseController {
	private Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired(required = true)
	private IUasAppService uasAppService;
	...
	private EUDataGridResult list(@RequestParam Map<String, Object> condition, @RequestParam(value = "page") Integer page,
		...
		try {
			res = uasAppService.selectList(condition, page, rows);
		} catch(Exception e) {
			logger.error("查询出错", e);
		}
		return res;
	...
	@ResponseBody
	public CustomResult update(UasApp record) {
		int ret = -1;
		try {
			ret = uasAppService.update(record);
		} catch(Exception e) {
			logger.error("更新出错", e);
		}
	...
	public CustomResult deleteRoles(String[] ids) {
		int ret =  -1;
		try {
			ret = uasAppService.deleteBatch(ids);
		} catch(Exception e) {
			logger.error("删除出错", e);
		}
	...
	@ResponseBody
	public CustomResult insert(UasApp record) {
		int ret =  -1;
		try {
		    ret =  uasAppService.insert(record);
		} catch (Exception e) {
			logger.error("保存出错", e);
		}
	...
	public List<Tree> getAppTree() {
		return uasAppService.getAppTree();
	}
}
