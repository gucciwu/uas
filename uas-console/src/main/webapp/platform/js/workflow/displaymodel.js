...
var pframe=$("#iframeParent" , parent.document);
modelId=pframe.attr('pid');
...
if (modelType == 'runtime') {
	if (historyModelId) {
    	...
    	modelUrl = ACTIVITI.CONFIG.contextRoot + '/workflow/tracking/' + modelId + '/model-json';//显示流程跟踪的页面是用这个url
	}
...
request.success(function(datajson, textStatus, jqXHR) {
	var data = JSON.parse(datajson); //change json string to object
